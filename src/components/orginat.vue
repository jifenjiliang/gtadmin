        ...
        <div class="box_c_boxy_c">
          <el-form :inline="true" :model="peopleDate" >
            ...
            <el-form-item label="行业">
              <el-select clearable v-model="Addclient.orgtIndustry" placeholder="请输入一级分类" >
                <el-option v-for="item in orgtIndustrys" :value="item" :key="item.id"></el-option>
              </el-select>
              <el-select clearable v-model="Addclient.orgsIndustry" placeholder="请输入二级分类" >
                <el-option v-for="item in orgsIndustrys" :value="item" :key="item.id"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="地区" >
              <el-select clearable v-model="Addclient.firstIndustry" placeholder="请输入省" >
                <el-option v-for="item in firstIndustrys" :value="item" :key="item.id"></el-option>
              </el-select>
              <el-select clearable v-model="Addclient.secondIndustry" placeholder="请输入市" >
                <el-option v-for="item in secondIndustrys" :value="item" :key="item.id"></el-option>
              </el-select>
              <el-select clearable v-model="Addclient.threeIndustry" placeholder="请输入区">
  ...
  export default {
    ...
    computed: {
      firstIndustrys: function() {
        ...
        if(!this.addressData[this.Addclient.firstIndustry]){
          this.Addclient.secondIndustry = "";
        }
        var c = []
      ...
      secondIndustrys: function() {
        if (!this.addressData || !this.Addclient.firstIndustry)
          return
        if(!this.addressData[this.Addclient.firstIndustry][this.Addclient.secondIndustry]){
          this.Addclient.threeIndustry = "";
        }
        var c = []
        for (var key in this.addressData[this.Addclient.firstIndustry]) {
          c.push(key)
        }
        return c
      },
      threeIndustrys: function() {
        if (!this.addressData || !this.Addclient.secondIndustry)
          return
        var c = []
        for (var key in this.addressData[this.Addclient.firstIndustry][this.Addclient.secondIndustry]) {
          c.push(key)
        }
        return c
      },
      ...
      orgsIndustrys:function () {
        if (!this.AaddressData || !this.Addclient.orgtIndustry)
          return

        var c = []
        for (var key in this.AaddressData[this.Addclient.orgtIndustry]) {
          c.push(key)
        }
        return c
      },
    ...
    methods:{
      ...
      search(){
        ...
        that.peopleDate.area = that.Addclient.firstIndustry + '_' + that.Addclient.secondIndustry + '_' + that.Addclient.threeIndustry;
        that.peopleDate.industy = that.Addclient.orgtIndustry + '_' + that.Addclient.orgsIndustry;
