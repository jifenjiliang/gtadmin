<template>
  <div item="zz" class="gintong-content" v-if="isshow">
          ...
          <div class="list_body" style="margin-right: 10px;">
            ...
            <div class="list_list">
              <dl v-for="(item, index) in organ" :key="item.id"  @click="getpeopleval(item,index)">
                ...
                <dd>组织<i v-show="item.show" class="el-icon-check"></i></dd>
              </dl>
          ...
          <div class="list_body">
            <div class="list_list">
              <dl v-for="(item, index) in saveorgan" :key="item.id" @click="delpeople(index, item)">
                <dt><img :src="item.organizationMini.logo" alt="">{{item.name}}</dt>
                <dd>组织<i  class="el-icon-close"></i></dd>
              </dl>
            </div>
          </div>
        </el-col>
  ...
  export default {
    ...
    methods:{
      initlistdate:function () {
        /* 初始化数据 */
        let vm = this;
        let url = '/cross/asso/pub/getNewRelatedResources/organ/2/'+ vm.index + '/20/' + vm.keywords + '/5';
      ...
      getpeopleval(obj, index) {
        let vm = this;
        if (vm.saveorgan.indexOf(obj) == -1) {
          vm.saveorgan.push(obj)
          obj.show = !obj.show;
        } else {
          vm.saveorgan.splice(vm.saveorgan.indexOf(obj), 1)
          obj.show = false;
        }
        vm.dealSaveDate(vm.saveorgan);
      },
