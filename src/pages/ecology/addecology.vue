<template>
  <div class="addecology">
    <section class="section">
      <div class="loation"><router-link to="/ecology">内容管理</router-link>>><router-link to="/ecology">生态推荐</router-link>>>添加热词</div>
  ...
  export default {
    ...
    data:function () {
      return {
        ...
        id:null,
        ket:null,
        serverapi: '/cross/organ/newWords/saveNewWords.json', // 发送
    ...
    methods:{
      ...
      getloadate:function () {
        var that =  this;
        that.$http.post('/cross/organ/newWords/listPosition.json',that.ket,{headers:{
          sessionID :that.sessionid,
          s:'web',
        }}).then(
          function (res) {
            this.form.options = res.data.responseData.listPosition;
          },function (res) {
            console.log(res)
          }
        )
...
//   保存数据
      onSave(){
        ...
        var localStorageuer = JSON.parse(localStorage.getItem('currentUser'))
        var that =  this;

        /*let  customerList = that.form.demandList.concat(that.form.customerList);
        let  knowledgeList =that.form.knowledgeList.concat(that.form.peopleList);
        let  relations = customerList.concat(knowledgeList);*/

        let AA = that.form.peopleList.concat(that.form.customerList);
        let BB = that.form.customerList2.concat(that.form.knowledgeList);
        ...
        var savepath = {
          ...
          name : that.form.name,
          status : 0, //用户状态
          userId : parseInt(localStorageuer.uid), // 用户
          userName : localStorageuer.name, //用户
          userType : 0, // 默认
          relation: relations
        };
