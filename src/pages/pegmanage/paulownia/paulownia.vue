<template>
  <div class="headlines">
    <div class="loation"><router-link to="/headlines">PGC内容管理</router-link> >>金桐精选</div>
      ...
      <div class="serch">
        <el-form ref="form"  :inline="true" :model="form" label-width="70px">
          ...
          <el-form-item>
            ...
            <el-button type="primary"  icon="edit" @click="Newadd">新增精选</el-button>
          </el-form-item>
        </el-form>
      </div>
      <!-- show table dates-->
  ...
  export default {
    ...
    data:function () {
      return {
        ...
        headlines:[],
        api:'/pub/PGCsearch',
        del:'/pub/deleteColumnDescRelation',
        total:0,
        index:1,
        form: {
          type:1,
    ...
    methods:{
      ...
      // Newadd
      Newadd(){
        this.$router.push({path: '/paulownia/added'})
      },
      //Seefun
      Seefun(key,val){
        this.$router.push({path: '/paulownia/see/' + val.sourceId + '?type=' + val.columnType})
      },
      //Editfun
      Editfun(key,val){
        this.$router.push({path: '/paulownia/edit/' + val.id})
        localStorage.setItem('jinxun',JSON.stringify(val));
      },
      //Delfun
      Delfun(key,val){
        let that = this;
